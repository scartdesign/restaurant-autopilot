...
begin
  ...
  v_estimated_base:=least(100,greatest(0,v_pre_repeat-coalesce(v.performance_boost,0)+coalesce(v.freshness_penalty,0)));

  return jsonb_build_object(
    'id',v.id,'trend_query',v.trend_query,'seed_query',v.seed_query,
    'final_score',v.opportunity_score,'estimated_base_score',v_estimated_base,
    'performance_boost',coalesce(v.performance_boost,0),'performance_samples',coalesce(v.performance_samples,0),
    'effectiveness_score',coalesce(v.effectiveness_score,50),'effectiveness_samples',coalesce(v.effectiveness_samples,0),
    'freshness_penalty',coalesce(v.freshness_penalty,0),'repeat_penalty',coalesce(v.repeat_penalty,0),
    'candidate_last_seen_at',v.candidate_last_seen_at,'last_seed_used_at',v.last_seed_used_at,'snoozed_until',v.snoozed_until,
    'auto_guard',case
      when v.status='snoozed' then 'snoozed'
...
create or replace function public.trend_opportunity_explain(p_id uuid)
returns jsonb
language sql
security invoker
set search_path to ''
as $function$
  select private.trend_opportunity_explain(p_id);
$function$;

revoke all on function public.trend_opportunity_explain(uuid) from public,anon;
grant execute on function public.trend_opportunity_explain(uuid) to authenticated;