...
revoke all on function private.restaurant_trend_auto_preview(uuid) from public,anon,authenticated;

create or replace function public.restaurant_trend_auto_preview(p_restaurant_id uuid)
returns jsonb
language sql
security invoker
set search_path to ''
as $function$
  select private.restaurant_trend_auto_preview(p_restaurant_id);
$function$;

revoke all on function public.restaurant_trend_auto_preview(uuid) from public,anon;
grant execute on function public.restaurant_trend_auto_preview(uuid) to authenticated;
