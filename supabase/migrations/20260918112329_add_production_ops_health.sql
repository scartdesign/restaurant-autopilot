...
begin
  ...
  select status,started_at,finished_at,error_message,metadata into v_latest
  from private.discovery_sync_runs where source='serpapi_google_trends' order by started_at desc limit 1;
  v_discovery:=jsonb_build_object(
    'provider_configured',v_serpapi,'provider_enabled',v_discovery_enabled,'cron_active',v_discovery_cron,
    'latest_status',coalesce(v_latest.status,'never'),'latest_started_at',v_latest.started_at,'latest_finished_at',v_latest.finished_at,
    'latest_error',v_latest.error_message,'latest_reason',v_latest.metadata->>'reason',
  ...
  select count(*) filter(where status='connected')::integer,count(*) filter(where status='expired')::integer,count(*) filter(where status='error')::integer
    into v_connected,v_expired,v_error from public.social_connections where provider='meta';
  select count(*) filter(where status='queued')::integer,count(*) filter(where status='queued' and attempt_count>0)::integer,
    count(*) filter(where status='failed')::integer,count(*) filter(where status='processing')::integer,
    count(*) filter(where status='processing' and updated_at<now()-interval '15 minutes')::integer,
    count(*) filter(where status='failed' and attempt_count>=3)::integer,count(*) filter(where insights_error is not null)::integer,max(published_at)
    into v_queued,v_retrying,v_failed,v_processing,v_stuck,v_maxed,v_insights_failed,v_last_published from public.social_publish_jobs;
  v_meta:=jsonb_build_object(
    'provider_configured',v_meta_app and v_meta_secret,'cron_active',v_meta_cron,'connected',coalesce(v_connected,0),
    'expired_connections',coalesce(v_expired,0),'error_connections',coalesce(v_error,0),'queued',coalesce(v_queued,0),
    'retrying',coalesce(v_retrying,0),'failed',coalesce(v_failed,0),'processing',coalesce(v_processing,0),
    'stuck_processing',coalesce(v_stuck,0),'max_attempts_failed',coalesce(v_maxed,0),'insights_failed',coalesce(v_insights_failed,0),
    'last_published_at',v_last_published,'auto_retry_max_attempts',3,'auto_retry_backoff_minutes',jsonb_build_array(5,20)
  );
  select coalesce(jsonb_agg(step),'[]'::jsonb) into v_blocking from (
    values
  ...
  ) as x(step) where step is not null;
  v_e2e:=jsonb_build_object(
    'serpapi_ready',v_serpapi and v_discovery_enabled and v_discovery_cron,
    'meta_provider_ready',v_meta_app and v_meta_secret and v_meta_cron,
...
revoke all on function private.admin_production_ops_health() from public,anon;
grant execute on function private.admin_production_ops_health() to authenticated;

create or replace function public.admin_production_ops_health()
returns jsonb
language sql
security invoker
set search_path to ''
as $function$
  select private.admin_production_ops_health();
$function$;

revoke all on function public.admin_production_ops_health() from public,anon;
grant execute on function public.admin_production_ops_health() to authenticated;
