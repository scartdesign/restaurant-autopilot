...
begin
  ...
  return coalesce((
    with restaurant_base as (
      ...
      select
        ...
        exists(
          ...
          where m.restaurant_id=r.id and m.is_active
        ) as has_active_menu_item,
        exists(
      ...
      select *,
        case
          ...
          when not has_active_menu_item then 'no_menu_item'
          else 'would_create'
        end as decision
      from resolved
    )
    select jsonb_agg(jsonb_build_object(
      ...
      'has_active_menu_item',has_active_menu_item
    ) order by name)
    from decided
  ),'[]'::jsonb);
end;
