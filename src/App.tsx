...
import { RestorappLogo } from './components/RestorappLogo'
...
function App() {
  ...
  return <div className="app-shell"><NetworkStatus/>
    ...
    <aside className="sidebar sidebar-pro"><div>
      <div className="brand-mark brand-mark-restorapp"><RestorappLogo surface="dark" tagline /></div>
...
function MaintenanceScreen({message,version,onSignOut}:{message:string|null;version:string;onSignOut:()=>Promise<void>}){
  return <div className="maintenance-screen"><div className="maintenance-card"><div className="maintenance-logo maintenance-logo-restorapp"><RestorappLogo surface="light" tagline /></div><span>RESTORAPP · v{version}</span><h1>Kratko održavanje.</h1><p>{message||'OWNER trenutno radi na sistemu. Tvoji podaci ostaju sačuvani i pristup će se vratiti čim održavanje bude završeno.'}</p><button className="secondary" onClick={onSignOut}><LogOut size={15}/> Odjavi se</button></div></div>
}
