...
export function NotificationsCenter({setNotice,onUnreadChanged,onNavigate}:{setNotice:(v:string)=>void;onUnreadChanged?:(count:number)=>void;onNavigate?:(target:'insights'|'billing'|'support'|'dashboard')=>void}){
  ...
  useEffect(()=>{void load();void loadDigestPreference()},[])
  ...
  async function loadDigestPreference(){const restaurantId=localStorage.getItem(ACTIVE_RESTAURANT_KEY);if(!restaurantId){setDigestEnabled(null);return}const{data,error}=await supabase.from('restaurants').select('trend_weekly_digest_enabled').eq('id',restaurantId).maybeSingle();if(error){setDigestEnabled(null);return}setDigestEnabled(data?.trend_weekly_digest_enabled!==false)}
  async function toggleDigest(){const restaurantId=localStorage.getItem(ACTIVE_RESTAURANT_KEY);if(!restaurantId||digestEnabled===null)return;const next=!digestEnabled;setDigestWorking(true);const{error}=await supabase.from('restaurants').update({trend_weekly_digest_enabled:next}).eq('id',restaurantId);if(error)setNotice(error.message);else{setDigestEnabled(next);setNotice(next?'Weekly Trend Brief je uključen za aktivni restoran.':'Weekly Trend Brief je isključen za aktivni restoran.')}setDigestWorking(false)}
  ...
  return <div className="notifications-page">
    ...
    {digestEnabled!==null&&<section className={'notification-preference-card '+(digestEnabled?'active':'')}><span className="notification-preference-icon"><CalendarDays size={19}/></span><div><small>TREND WEEKLY BRIEF</small><strong>{digestEnabled?'Nedeljni trend sažetak je uključen':'Nedeljni trend sažetak je isključen'}</strong><p>Jednom nedeljno dobijaš najviše 3 najbolje aktivne Trend Radar prilike za trenutno izabrani restoran. Samo in-app — bez email spama.</p></div><button type="button" className={digestEnabled?'toggle active':'toggle'} onClick={()=>void toggleDigest()} disabled={digestWorking}><span>{digestEnabled?'UKLJUČEN':'ISKLJUČEN'}</span></button></section>}
