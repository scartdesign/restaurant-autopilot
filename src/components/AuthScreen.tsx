    ...
    <div className="auth-page auth-page-wow">
      <section className="auth-showcase" style={{ backgroundImage: `linear-gradient(180deg, rgba(8,13,10,.08), rgba(8,13,10,.88)), url(${LOGIN_FOOD})` }}>
        <div className="auth-showcase-top">
          <div className="auth-showcase-brand"><span><ChefHat size={22} /></span><strong>Restaurant Autopilot</strong></div>
          <span className="auth-live"><i /> AI MARKETING SYSTEM</span>
        </div>
        <div className="auth-showcase-copy">
          <span className="auth-overline">OD MENIJA DO OBJAVE</span>
          <h2>Tvoj restoran izgleda dobro.<br /><em>Sada neka tako izgleda i online.</em></h2>
          <p>Fotografije, tekstovi, lokalni discovery, kampanje i gotovi vizuali — u jednom toku.</p>
          <div className="auth-showcase-pills"><span><Camera size={15} /> Realne fotografije</span><span><CalendarDays size={15} /> Nedeljni plan</span><span><Hash size={15} /> Smart discovery</span></div>
        </div>
        <div className="auth-floating-card">
          <div className="auth-floating-head"><div><span>Sledeća objava</span><strong>Pizza Capricciosa</strong></div><span className="auth-score">94/100</span></div>
          <div className="auth-floating-meta"><span><CheckCircle2 size={14} /> spremno</span><span>Feed 4:5</span><span>18:30</span></div>
        </div>
      </section>
      ...
      <section className="auth-form-zone">
        <div className="auth-card auth-card-pro auth-card-wow">
          ...
          <h1>{mode === 'login' ? <>Dobrodošao<br />nazad.</> : <>Pokreni svoj<br />Autopilot.</>}</h1>
          <p className="muted">{mode === 'login' ? 'Uđi u komandni centar svog restorana.' : 'Napravi nalog i pripremi prvi sadržaj za nekoliko minuta.'}</p>
          ...
          <form onSubmit={submit}>
            ...
            <button className="primary full auth-submit" disabled={working}>{working ? 'Sačekaj…' : mode === 'login' ? 'Prijavi se' : 'Napravi nalog'} <ArrowRight size={17} /></button>
          </form>
