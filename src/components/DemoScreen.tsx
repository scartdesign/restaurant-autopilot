...
type DemoDesign={id:string;dishId:string;title:string;text:string;template:RestaurantTemplateId;image:string;price:string;badge:string;primary:string;accent:string;textSlots:Record<string,string>;itemSlots:TemplateItemSlot[]}
...
function DemoContent({notify,setTab}:{notify:(value:string)=>void;setTab:(tab:DemoTab)=>void}) {
  ...
  const[designs,setDesigns]=useState<DemoDesign[]>(()=>demoPosts.map((post,index)=>({
    ...
    price:index===0?'890 RSD':index===1?'940 RSD':index===2?'520 RSD':'690 RSD',badge:post.type==='PROMO'?'20% OFF':'',primary:'#073c38',accent:'#ef7d3a',textSlots:defaultTextSlots((['luxe','editorial','minimal','bold'][index] as RestaurantTemplateId)),itemSlots:defaultItemSlots((['luxe','editorial','minimal','bold'][index] as RestaurantTemplateId)),
  })))
  ...
  function startDish(item:DemoDish){
    setSelectedDishId(item.id);setHeadline(item.name);setText(item.description);setPriceText(item.price);setBadge('');setTemplate('luxe');setPrimaryColor('#073c38');setAccentColor('#ef7d3a');setTextSlots({...defaultTextSlots('luxe'),smallDesc:item.description,buttonText:'Rezerviši sto'});setItemSlots(defaultItemSlots('luxe'));setEditingDesignId('');setStudioTab('templates')
  ...
  function saveDesign(){
    ...
    const data={id:editingDesignId||'design-'+Date.now(),dishId:selectedDish.id,title:headline.trim(),text:text.trim(),template,image:selectedDish.image,price:priceText,badge,primary:primaryColor,accent:accentColor,textSlots:{...textSlots},itemSlots:itemSlots.map(item=>({...item}))}
  ...
  return <div className="dish-template-studio demo-dish-template-studio">
    ...
    {studioTab==='templates'&&<section className="dts-template-screen">
      ...
      <aside className="dts-composer"><div className="dts-composer-head"><span>OBJAVA</span><strong>{editingDesignId?'Izmeni objavu':'Dovrši objavu'}</strong></div>
        ...
        <RestaurantTemplateCanvas className="dts-live-preview" template={template} image={selectedDish.image} headline={headline} text={text} price={priceText} badge={badge} cta={cta||'BUY'} primary={primaryColor} accent={accentColor} logoUrl={demoLogo} format={format} textSlots={textSlots} itemSlots={itemSlots}/>
