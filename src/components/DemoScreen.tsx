...
function DemoContent({notify,setTab}:{notify:(value:string)=>void;setTab:(tab:DemoTab)=>void}) {
  ...
  const[editorPanel,setEditorPanel]=useState<'text'|'style'|'fonts'>('text')
    ...
    {studioTab==='templates'&&<section className="dts-template-screen">
      ...
      <aside className="dts-composer dts-composer-premium">
        ...
        {editorPanel==='text'&&<div className="dts-editor-panel">
          ...
          <details className="dts-advanced-copy"><summary>Dodatni tekstovi šablona <span>{demoContentTemplates.find(item=>item.id===template)?.name}</span></summary>
            <div className="dts-template-text-editor"><div className="dts-template-text-head"><div><span>TEKSTOVI NA DIZAJNU</span><small>Menjaj samo ono što ti treba.</small></div><button type="button" onClick={()=>chooseDemoTemplate(template)}>Vrati</button></div><div className="dts-template-text-fields">{selectedTemplateConfig.textSlots.map(slot=><label key={slot.key}>{slot.label}{slot.multiline?<textarea rows={2} maxLength={slot.maxLength} value={textSlots[slot.key]??slot.defaultValue} onChange={e=>setTextSlots(current=>({...current,[slot.key]:e.target.value}))}/>:<input maxLength={slot.maxLength} value={textSlots[slot.key]??slot.defaultValue} onChange={e=>setTextSlots(current=>({...current,[slot.key]:e.target.value}))}/>}</label>)}</div>{selectedTemplateConfig.itemSlots?.length?<div className="dts-item-slot-editor"><div className="dts-item-slot-title"><span>STAVKE U MENIJU</span><small>Naziv i cena svake stavke.</small></div>{itemSlots.map((item,index)=><div className="dts-item-slot-row" key={index}><b>{index+1}</b><input value={item.title} onChange={e=>setItemSlots(current=>current.map((entry,i)=>i===index?{...entry,title:e.target.value}:entry))}/><input value={item.price} onChange={e=>setItemSlots(current=>current.map((entry,i)=>i===index?{...entry,price:e.target.value}:entry))}/></div>)}</div>:null}</div>
          </details>
        </div>}

        {editorPanel==='style'&&<div className="dts-editor-panel"><div className="dts-color-editor premium-box"><div className="dts-color-title"><span>PALETA</span><small>Jedan klik menja ceo vizuelni identitet.</small></div><div className="dts-palette-row">{demoColorPalettes.map(palette=><button type="button" key={palette.name} className={primaryColor===palette.primary&&accentColor===palette.accent?'active':''} onClick={()=>{setPrimaryColor(palette.primary);setAccentColor(palette.accent)}}><i style={{background:palette.primary}}/><i style={{background:palette.accent}}/><span>{palette.name}</span></button>)}</div><div className="dts-color-pickers"><label>Glavna<input type="color" value={primaryColor} onChange={e=>setPrimaryColor(e.target.value)}/><span>{primaryColor}</span></label><label>Akcent<input type="color" value={accentColor} onChange={e=>setAccentColor(e.target.value)}/><span>{accentColor}</span></label></div></div></div>}

        {editorPanel==='fonts'&&<div className="dts-editor-panel"><div className="dts-font-editor premium-box"><div className="dts-font-title"><span>TIPOGRAFIJA</span><small>Probrane kombinacije koje rade u restoran dizajnu.</small></div><div className="dts-font-group"><strong>Osnovni font</strong><div className="dts-font-options">{baseFontOptions.map(font=><button type="button" key={font.id} className={baseFont===font.id?'active':''} onClick={()=>setBaseFont(font.id)}><b style={{fontFamily:baseFontStack(font.id)}}>{font.sample}</b><span>{font.name}</span></button>)}</div></div><div className="dts-font-group"><strong>Pisani font</strong><div className="dts-font-options script">{scriptFontOptions.map(font=><button type="button" key={font.id} className={scriptFont===font.id?'active':''} onClick={()=>setScriptFont(font.id)}><b style={{fontFamily:scriptFontStack(font.id)}}>{font.sample}</b><span>{font.name}</span></button>)}</div></div></div></div>}
