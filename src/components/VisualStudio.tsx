...
function buildSvg({width,height,design,restaurantName,location,price,backgroundData,logoData}:{width:number;height:number;design:DesignState;restaurantName:string;location:string;price:string;backgroundData:string|null;logoData:string|null}){
  ...
  const head=headline.map((line,i)=>`<text x="${m}" y="${y+i*headlineStep}" font-family="${fontFamily}" font-size="${headlineSize}" font-weight="${serif?700:900}" letter-spacing="${headlineTracking}" fill="${lightCard?design.primaryColor:'#fff'}">${esc(line)}</text>`).join(''), bodyStart=y+headline.length*headlineStep+(story?42:30), bodySize=(story?35:27)*design.sublineScale, bodySvg=body.map((line,i)=>`<text x="${m}" y="${bodyStart+i*bodySize*1.38}" font-family="Arial,sans-serif" font-size="${bodySize}" fill="${lightCard?'#4f5b52':'#fff'}" opacity=".9">${esc(line)}</text>`).join(''), ctaY=bodyStart+Math.max(1,body.length)*bodySize*1.38+(story?56:38), ctaW=(story?400:330)*Math.min(1.25,design.ctaScale),ctaH=(story?82:64)*design.ctaScale
  ...
  const priceSvg=price&&design.priceVisible?`<rect x="${m}" y="${y-(story?96:72)}" rx="${priceH/2}" width="${priceW}" height="${priceH}" fill="${design.accentColor}"/><text x="${m+priceW/2}" y="${y-(story?96:72)+priceH*.68}" text-anchor="middle" font-family="Arial" font-size="${priceFont}" font-weight="900" fill="${design.primaryColor}">${esc(price)}</text>`:'', cta=`<rect x="${m}" y="${ctaY}" rx="999" width="${ctaW}" height="${ctaH}" fill="${lightCard?design.primaryColor:design.accentColor}"/><text x="${m+ctaW/2}" y="${ctaY+ctaH*.66}" text-anchor="middle" font-family="Arial" font-size="${(story?30:24)*design.ctaScale}" font-weight="900" fill="${lightCard?'#fff':design.primaryColor}">${esc(design.cta||'Svrati danas')} →</text>`
