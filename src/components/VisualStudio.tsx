...
function buildSvg({width,height,design,restaurantName,location,price,backgroundData,logoData}:{width:number;height:number;design:DesignState;restaurantName:string;location:string;price:string;backgroundData:string|null;logoData:string|null}){
  ...
  const priceSvg=price&&design.priceVisible?`<rect x="${m}" y="${y-(story?96:72)}" rx="999" width="${story?270:220}" height="${story?62:50}" fill="${design.accentColor}"/><text x="${m+(story?135:110)}" y="${y-(story?54:38)}" text-anchor="middle" font-family="Arial" font-size="${story?29:23}" font-weight="900" fill="${design.primaryColor}">${esc(price)}</text>`:'', cta=`<rect x="${m}" y="${ctaY}" rx="999" width="${ctaW}" height="${ctaH}" fill="${design.template==='minimal'?design.primaryColor:design.accentColor}"/><text x="${m+ctaW/2}" y="${ctaY+(story?53:42)}" text-anchor="middle" font-family="Arial" font-size="${story?30:24}" font-weight="900" fill="${design.template==='minimal'?'#fff':design.primaryColor}">${esc(design.cta||'Svrati danas')} →</text>`
