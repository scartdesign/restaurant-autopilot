...
export function VisualStudio({ restaurant, posts, menuItems, setNotice }: { restaurant:Restaurant; posts:Post[]; menuItems:MenuItem[]; setNotice:(value:string)=>void }) {
  ...
  async function saveDesign() {
    ...
    const meta = { ...(selected.generation_meta || {}), image_url: design.imageUrl, visual_design: {
      ...
      copy_position:design.copyPosition, font_pair:design.fontPair, price_visible:design.priceVisible, saved_at:new Date().toISOString(),
    }}
...
function designFromPost(post:Post|undefined, menu:MenuItem[], r:Restaurant):DesignState{
  const brand=brandDefaults(r); if(!post)return{template:'editorial',headline:'',subline:'',cta:'Svrati danas',format:'feed',imageUrl:null,photoPosition:'center',copyPosition:'bottom',fontPair:'editorial',priceVisible:true,...brand}
...
function buildPromoGridSvg({width,height,design,restaurantName,location,logoData,items}:{width:number;height:number;design:DesignState;restaurantName:string;location:string;logoData:string|null;items:PromoGridExportItem[]}){
  ...
  const card=(item:PromoGridExportItem,x:number,y:number,w:number,h:number,index:number)=>{
    ...
    const titleSize=index===0?(story?54:42):(story?29:23)
...
function buildSvg({width,height,design,restaurantName,location,price,backgroundData,logoData}:{width:number;height:number;design:DesignState;restaurantName:string;location:string;price:string;backgroundData:string|null;logoData:string|null}){
  ...
  const priceSvg=price&&design.priceVisible?`<rect x="${m}" y="${y-(story?96:72)}" rx="999" width="${story?270:220}" height="${story?62:50}" fill="${design.accentColor}"/><text x="${m+(story?135:110)}" y="${y-(story?54:38)}" text-anchor="middle" font-family="Arial" font-size="${story?29:23}" font-weight="900" fill="${design.primaryColor}">${esc(price)}</text>`:'', cta=`<rect x="${m}" y="${ctaY}" rx="999" width="${ctaW}" height="${ctaH}" fill="${lightCard?design.primaryColor:design.accentColor}"/><text x="${m+ctaW/2}" y="${ctaY+(story?53:42)}" text-anchor="middle" font-family="Arial" font-size="${story?30:24}" font-weight="900" fill="${lightCard?'#fff':design.primaryColor}">${esc(design.cta||'Svrati danas')} →</text>`
