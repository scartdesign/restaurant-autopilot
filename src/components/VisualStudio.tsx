...
function buildPromoGridSvg({width,height,design,restaurantName,location,logoData,items}:{width:number;height:number;design:DesignState;restaurantName:string;location:string;logoData:string|null;items:PromoGridExportItem[]}){
  ...
  const card=(item:PromoGridExportItem,x:number,y:number,w:number,h:number,index:number)=>{
    ...
    const price=item.price&&design.priceVisible?`<rect x="${tx}" y="${ty+20}" width="${index===0?190:132}" height="${index===0?48:34}" rx="17" fill="${design.accentColor}"/><text x="${tx+(index===0?95:66)}" y="${ty+(index===0?53:43)}" text-anchor="middle" font-family="Arial" font-size="${index===0?22:15}" font-weight="900" fill="${design.primaryColor}">${esc(item.price)}</text>`:''
    return `<g><clipPath id="clip${index}"><rect x="${x}" y="${y}" width="${w}" height="${h}" rx="24"/></clipPath><g clip-path="url(#clip${index})">${img}<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="url(#gridShade)"/></g><text x="${tx}" y="${y+(index===0?44:29)}" font-family="Arial" font-size="${index===0?16:11}" font-weight="900" letter-spacing="2" fill="${design.accentColor}">${label}</text><text x="${tx}" y="${ty}" font-family="Arial" font-size="${titleSize}" font-weight="900" fill="#fff">${esc(shorten(item.name,index===0?26:20))}</text>${price}</g>`
  }
  let cards=''
  if(story){
  ...
  const footerLineGap=(story?60:50)*design.headlineScale
  const headline=wrap(design.headline||'Ukus koji se pamti.',story?24:30,2).map((line,i)=>`<text x="${m}" y="${footerY+88+i*footerLineGap}" font-family="Arial Black,Arial" font-size="${footerHeadlineSize}" font-weight="900" fill="#fff">${esc(line)}</text>`).join('')
...
function buildSvg({width,height,design,restaurantName,location,price,backgroundData,logoData}:{width:number;height:number;design:DesignState;restaurantName:string;location:string;price:string;backgroundData:string|null;logoData:string|null}){
  ...
  const priceSvg=price&&design.priceVisible?`<rect x="${m}" y="${y-(story?96:72)}" rx="999" width="${story?270:220}" height="${story?62:50}" fill="${design.accentColor}"/><text x="${m+(story?135:110)}" y="${y-(story?54:38)}" text-anchor="middle" font-family="Arial" font-size="${story?29:23}" font-weight="900" fill="${design.primaryColor}">${esc(price)}</text>`:'', cta=`<rect x="${m}" y="${ctaY}" rx="999" width="${ctaW}" height="${ctaH}" fill="${lightCard?design.primaryColor:design.accentColor}"/><text x="${m+ctaW/2}" y="${ctaY+ctaH*.66}" text-anchor="middle" font-family="Arial" font-size="${(story?30:24)*design.ctaScale}" font-weight="900" fill="${lightCard?'#fff':design.primaryColor}">${esc(design.cta||'Svrati danas')} →</text>`
