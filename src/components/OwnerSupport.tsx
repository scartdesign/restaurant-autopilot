...
export function OwnerSupport({setNotice}:{setNotice:(v:string)=>void}){
  ...
  async function save(){if(!selected)return;setWorking(true);const now=new Date().toISOString();const{error}=await supabase.from('support_tickets').update({status,admin_note:note.trim()||null,updated_at:now,resolved_at:status==='resolved'?now:null}).eq('id',selected.id);if(error)setNotice(error.message);else{setNotice('Support zahtev je ažuriran. Kupac dobija odgovor u aplikaciji.');setSelected(null);await load()}setWorking(false)}
