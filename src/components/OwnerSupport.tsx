...
export function OwnerSupport({setNotice}:{setNotice:(v:string)=>void}){
  ...
  async function save(){if(!selected)return;setWorking(true);const{error}=await supabase.rpc('admin_update_support_ticket',{p_ticket_id:selected.id,p_status:status,p_admin_note:note||null});if(error)setNotice(error.message);else{setNotice('Support zahtev je ažuriran. Kupac dobija odgovor u aplikaciji.');setSelected(null);await load()}setWorking(false)}
